...
class DDPG(object):
    def __init__(self, network, target_network, memory, ob_space, ac_space, param_noise=None, action_noise=None,
        ...
        for k, v in self.ac_space.spaces.items():
            self.acts[k] = network.phs[k]

        self.terminals1 = tf.placeholder(tf.float32, shape=(None, 1), name='terminals1')
        self.rewards = tf.placeholder(tf.float32, shape=(None, None, 1), name='rewards')
        self.critic_target = tf.placeholder(tf.float32, shape=(None, None, 1), name='critic_target')
        ...
        self.normalized_critic_tf = network.value0
        self.critic_tf = denormalize(tf.clip_by_value(self.normalized_critic_tf, self.return_range[0], self.return_range[1]), self.ret_rms)
        self.normalized_critic_with_actor_tf = network.value1
        self.critic_with_actor_tf = denormalize(tf.clip_by_value(self.normalized_critic_with_actor_tf, self.return_range[0], self.return_range[1]), self.ret_rms)
        Q_obs1 = denormalize(target_network.value1, self.ret_rms)
    ...
    def setup_param_noise(self, obs0):
        ...
        self.perturbed_actor_tf = param_noise_actor.actions
        ...
        adaptive_actor_tf = adaptive_param_noise_actor.actions
    ...
    def setup_actor_optimizer(self):
        ...
        all_actor_vars = self.network.actor_variables
        actor_shapes = [var.get_shape().as_list() for var in all_actor_vars]
        actor_nb_params = sum([reduce(lambda x, y: x * y, shape) for shape in actor_shapes])
        logger.info('  actor shapes: {}'.format(actor_shapes))
        logger.info('  actor params: {}'.format(actor_nb_params))
        self.actor_grads = U.flatgrad(self.actor_loss, all_actor_vars, clip_norm=self.clip_norm)
        self.actor_optimizer = MpiAdam(var_list=all_actor_vars, beta1=0.9, beta2=0.999, epsilon=1e-08)

    def setup_critic_optimizer(self):
        logger.info('setting up critic optimizer')
        normalized_critic_target_tf = tf.clip_by_value(normalize(self.critic_target, self.ret_rms), self.return_range[0], self.return_range[1])
        self.critic_loss = tf.reduce_mean(tf.square(self.normalized_critic_tf - normalized_critic_target_tf))
        all_critic_vars = self.network.critic_variables
        if self.critic_l2_reg > 0.:
            critic_reg_vars = [var for var in all_critic_vars if var.name.endswith('/kernel:0') and 'output' not in var.name]
        ...
        critic_shapes = [var.get_shape().as_list() for var in all_critic_vars]
        critic_nb_params = sum([reduce(lambda x, y: x * y, shape) for shape in critic_shapes])
        logger.info('  critic shapes: {}'.format(critic_shapes))
        logger.info('  critic params: {}'.format(critic_nb_params))
        self.critic_grads = U.flatgrad(self.critic_loss, all_critic_vars, clip_norm=self.clip_norm)
        self.critic_optimizer = MpiAdam(var_list=all_critic_vars, beta1=0.9, beta2=0.999, epsilon=1e-08)
    ...
    def train(self):
        ...
        if self.normalize_returns and self.enable_popart:
            ...
            for r in batch['rewards']:
                br.append([r])
            feed_dict = {self.obs1[k] : v for k, v in batch_obs_to_dictObs(batch['obs1']).items()}
            extra_feed_dict = {
                self.rewards : br,
                self.terminals1 : batch['terminals1'].astype('float32'),
            }
        ...
        feed_dict = {self.acts[k] : v for k, v in batch_act_to_dictAct(batch['actions']).items()}
        obs_feed_dict = {self.obs0[k] : v for k, v in batch_obs_to_dictObs(batch['obs0']).items()}
        extra_feed_dict = {
            self.critic_target : target_Q,
        }
        feed_dict.update(obs_feed_dict)
        feed_dict.update(extra_feed_dict)
    ...
    def get_stats(self):
        ...
        feed_dict = {self.acts[k] : v for k, v in batch_act_to_dictAct(self.stats_sample['actions']).items()}
        obs_feed_dict = {self.obs0[k] : v for k, v in batch_obs_to_dictObs(self.stats_sample['obs0']).items()}
        feed_dict.update(obs_feed_dict)
