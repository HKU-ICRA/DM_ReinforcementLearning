...
def learn(env,
    ...
    network = SacPolicy(scope="sac", ob_space=ob_space, ac_space=ac_space, network_spec=network_spec,
                 v_network_spec=vnetwork_spec, v_network_spec2=vnetwork_spec2,
    ...
    target_network = SacPolicy(scope="sac_target", ob_space=ob_space, ac_space=ac_space, network_spec=network_spec,
                 v_network_spec=vnetwork_spec, v_network_spec2=vnetwork_spec2,
                 stochastic=False, reuse=False, build_act=True,
                 trainable_vars=None, not_trainable_vars=None,
                 gaussian_fixed_var=False, weight_decay=0.0, ema_beta=0.99999,
    ...
    for k, v in ob_space.spaces.items():
        obs0[k] = network.phs[k]

    # obs1
    obs1 = dict()
    for k, v in ob_space.spaces.items():
        obs1[k] = target_network.phs[k]

    # actions
    acts = dict()
    for k, v in ac_space.spaces.items():
        acts[k] = network.phs[k]

    self.dones = tf.placeholder(tf.float32, shape=(None, 1), name='dones')
    self.rewards = tf.placeholder(tf.float32, shape=(None, None, 1), name='rewards')

    # min double-q
    min_q_pi = tf.minimum(network.q1_pi, network.q2_pi)
