...
class DummyVecEnv(VecEnv):
    ...
    def __init__(self, env_fns):
        ...
        self.buf_obs = { k: np.zeros((self.num_envs, self.n_actors,) + tuple(shapes[k]), dtype=dtypes[k]) for k in self.keys }
        self.buf_dones = np.zeros((self.num_envs,), dtype=np.bool)
        self.buf_rews  = np.zeros((self.num_envs, env.n_actors,), dtype=np.float32)
